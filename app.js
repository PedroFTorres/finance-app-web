...
async function recalcularSaldo(conta_id){
 ...
 let c=0,d=0;
 (movs||[]).forEach(m=>{const v=Number(m.valor||0);if(m.tipo==="credito")c+=v;else d+=v;});
 const sf=si+c-d;
 await supabase.from("contas_bancarias").update({saldo_atual:sf}).eq("id",conta_id);
 return sf;
}
...
async function refreshLancamentos(){
 ...
 const [R,D]=await Promise.all([
  supabase.from("receitas").select("*").eq("user_id",currentUser.id).eq("conta_id",conta_id).gte("data",inicio).lte("data",fim).order("data"),
  supabase.from("despesas").select("*").eq("user_id",currentUser.id).eq("conta_id",conta_id).gte("data",inicio).lte("data",fim).order("data")
 ]);
 ...
 let tr=0,td=0;
 (R.data||[]).forEach(i=>{tr+=Number(i.valor||0);listReceitas.appendChild(buildLancItem(i,"receita"));});
 (D.data||[]).forEach(i=>{td+=Number(i.valor||0);listDespesas.appendChild(buildLancItem(i,"despesa"));});
...
async function loadDashboard(){
 ...
 const des=await supabase.from("despesas").select("*").eq("user_id",currentUser.id).gte("data",inicio).lte("data",fim);

 const tr=(rec.data||[]).reduce((s,x)=>s+Number(x.valor||0),0);
 const td=(des.data||[]).reduce((s,x)=>s+Number(x.valor||0),0);

 document.getElementById("dash-period").textContent=`${mes}/${ano}`;
 document.getElementById("dash-receber").textContent=formatReal(tr);
 document.getElementById("dash-pagar").textContent=formatReal(td);
 document.getElementById("dash-saldo-atual").textContent=formatReal(tr-td);
...
function subscribeToChanges(){
 supabase.channel("rec").on("postgres_changes",{event:"*",schema:"public",table:"receitas"},()=>refreshLancamentos()).subscribe();
 supabase.channel("des").on("postgres_changes",{event:"*",schema:"public",table:"despesas"},()=>refreshLancamentos()).subscribe();
 supabase.channel("mov").on("postgres_changes",{event:"*",schema:"public",table:"movimentacoes"},()=>renderExtrato()).subscribe();
}
